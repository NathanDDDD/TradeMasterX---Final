...
class KillSwitch:
        ...
        try:
            ...
            if self.kill_switch_file.exists():
                with open(self.kill_switch_file, 'r') as f:
                    state = json.load(f)
                    self._kill_switch_active = state.get('kill_switch_active', True)  # Default to safe state
                    self._live_trading_enabled = state.get('live_trading_enabled', False)
            else:
                # Default to safe state
                self._kill_switch_active = True  # Kill switch ON by default
                self._live_trading_enabled = False
        ...
        try:
            if self.config_file.exists():
                ...
                if not demo_mode or live_mode:
                    self.logger.critical("[CRITICAL] CRITICAL: Unsafe configuration detected!")
    ...
    def is_active(self) -> bool:
        """Alias for is_kill_switch_active() - Check if kill switch is currently active"""
    ...
    def _emergency_notification(self, reason: str):
        ...
        # This could be extended to send emails, SMS, webhooks, etc.        self.logger.critical("[EMERGENCY] EMERGENCY NOTIFICATION SYSTEM ACTIVATED")
