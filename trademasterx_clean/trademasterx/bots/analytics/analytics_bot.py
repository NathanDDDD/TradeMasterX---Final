...
class AnalyticsBot(BaseBot):
    ...
    def _prepare_accuracy_chart_data(self) -> List[Dict[str, Any]]:
        ...
        for conf_level, accuracy in self.prediction_summary.confidence_breakdown.items():
            ...
            })(chart_data, key=lambda x: x["confidence_level"])
    ...
    async def analyze_market(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                "win_rate": (trades_df['result'] == 'WIN').mean() if 'result' in trades_df.columns else 0,
                "avg_pnl": trades_df['pnl'].mean() if 'pnl' in trades_df.columns else 0,
                "market_volatility": trades_df['pnl'].std() if 'pnl' in trades_df.columns else 0,
                "analysis_timestamp": datetime.now().isoformat()
            }
            
            # Add trend analysis if we have enough data
            if len(trades_df) >= 10:
                recent_pnl = trades_df['pnl'].tail(10).mean() if 'pnl' in trades_df.columns else 0
                overall_pnl = trades_df['pnl'].mean() if 'pnl' in trades_df.columns else 0
                analysis["trend"] = "bullish" if recent_pnl > overall_pnl else "bearish"
            else:
                analysis["trend"] = "neutral"
            
            self.logger.info(f"Market analysis complete - Trend: {analysis.get('trend', 'unknown')}")
            return analysis
...
# No circular imports - registration happens in __init__.py
