...
class ObserverAgent:
        ...
        try:
            ...
            if data_dir.exists():
                ...
                if trade_files:
                    ...
                    if recent_file.stat().st_size <= 1:
                        self.logger.debug(f"Trade file {recent_file} is empty or too small")
                        return trades
                    ...
                    try:
                        ...
                        if df.empty or len(df.columns) == 0:
                            self.logger.debug(f"No valid data in trade file {recent_file}")
                            return trades
                        
                        # Check if required columns exist
                        if 'timestamp' not in df.columns:
                            self.logger.warning(f"Trade file {recent_file} missing timestamp column")
                            return trades
                        ...
                        self.logger.debug(f"Trade file {recent_file} has no data to parse")
                        return trades
                    except pd.errors.ParserError as e:
                        self.logger.warning(f"Failed to parse trade file {recent_file}: {e}")
                        return trades
    ...
    def observe_trade(self, trade_data: Dict[str, Any]) -> Dict[str, Any]:
        """Observe and log a trade - main interface method"""
        try:
            ...
            if self._is_anomaly(metrics):
                self._log_anomaly(metrics)
                self.logger.warning(f"Anomaly detected in trade {trade_data.get('id', 'unknown')}")
            
            # Add to trade buffer
            self.trade_buffer.append(metrics)
            ...
            return {
                'success': True,
                'metrics': metrics,
                'anomaly_detected': self._is_anomaly(metrics)
            }
            
        except Exception as e:
            self.logger.error(f"Failed to observe trade: {e}")
            return {
                'success': False,
                'error': str(e)
            }
