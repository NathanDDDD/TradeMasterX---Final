...
class ReinforcementEngine:
    ...
    def record_performance(self, strategy: str, performance_data: Dict[str, Any]):
        """Record performance data for a strategy - test interface method"""
        try:
            # Create trade-like data structure
            trade_data = {
                'strategy': strategy,
                'bot_name': strategy,
                'actual_return': performance_data.get('return', 0),
                'confidence': 0.8,  # Default confidence
                'timestamp': datetime.now().isoformat()
            }
            
            # Use existing method
            self.record_trade_performance(trade_data)
            
            self.logger.info(f"Recorded performance for strategy {strategy}")
